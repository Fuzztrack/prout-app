...
DECLARE
  v_method TEXT;
BEGIN
  -- Récupérer la valeur de method explicitement
  v_method := COALESCE(NULLIF(TRIM(NEW.method), ''), 'contact');
  
  -- Si method est NULL ou vide, définir à 'contact' par défaut
  IF NEW.method IS NULL OR TRIM(NEW.method) = '' THEN
    NEW.method := 'contact';
    v_method := 'contact';
  ELSE
    NEW.method := TRIM(NEW.method);
    v_method := TRIM(NEW.method);
  END IF;
